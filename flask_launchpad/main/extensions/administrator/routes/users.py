...
from ....builtins.functions.memberships import get_company_names
...
def users():
    if request.method == "GET":
        ...
        li_companies = get_company_names(session["user_id"])

        if "__global__" in li_companies:
            all_users = sql_do.query(FlUser).all
        else:
            get_users_from_company = sql_do.query(FlCompanyMembership.user_id).filter(
                FlCompanyMembership.company_id.in_(li_companies)
            ).all()
